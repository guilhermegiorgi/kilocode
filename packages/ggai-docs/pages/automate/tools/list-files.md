...
The `list_files` tool displays the files and directories within a specified location. It helps GG.AI understand your project structure and navigate your codebase effectively.
...
- When GG.AI needs to understand your project structure
- When GG.AI explores what files are available before reading specific ones
- When GG.AI maps a codebase to better understand its organization
- Before using more targeted tools like `read_file` or `search_files`
- When GG.AI needs to check for specific file types (like configuration files) across a project
...
- Marks files ignored by `.kilocodeignore` with a lock symbol (🔒) when `showGGAIIgnoredFiles` is enabled
...
6. **Formatting**:
   ...
   - Marks ignored files with a lock symbol (🔒) when `showGGAIIgnored` is enabled
...
- Files ignored by `.kilocodeignore` are marked with a lock symbol (🔒) when `showGGAIIgnored` is enabled
...
When `.kilocodeignore` files are used and `showGGAIIgnored` is enabled:
...
- When starting a new task, GG.AI may list the project files to understand its structure before diving into specific code.
- When asked to find specific types of files (like all JavaScript files), GG.AI first lists directories to know where to look.
- When providing recommendations for code organization, GG.AI examines the current project structure first.
- When setting up a new feature, GG.AI lists related directories to understand the project conventions.
