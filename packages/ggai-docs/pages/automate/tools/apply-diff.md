...
- When GG.AI needs to make precise changes to existing code without rewriting entire files.
...
2.  **GGAIIgnore Check**: Validates if the target file path is allowed by `.kilocodeignore` rules.
...
GG.AI uses this strategy for applying diffs:
