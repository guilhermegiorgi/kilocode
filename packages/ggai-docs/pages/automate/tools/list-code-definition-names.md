...
The `list_code_definition_names` tool provides a structural overview of your codebase by listing code definitions from source files at the top level of a specified directory. It helps GG.AI understand code architecture by displaying line numbers and definition snippets.
...
- When GG.AI needs to understand your codebase architecture quickly
- When GG.AI needs to locate important code constructs across multiple files
...
- When starting a new task, GG.AI first lists key code definitions to understand the overall structure of your project.
- When planning refactoring work, GG.AI uses this tool to identify classes and functions that might be affected.
- When exploring unfamiliar codebases, GG.AI maps the important code constructs before diving into implementation details.
- When adding new features, GG.AI identifies existing patterns and relevant code definitions to maintain consistency.
- When troubleshooting bugs, GG.AI maps the codebase structure to locate potential sources of the issue.
- When planning architecture changes, GG.AI identifies all affected components across files.
