...
The `execute_command` tool runs CLI commands on the user's system. It allows GG.AI to perform system operations, install dependencies, build projects, start servers, and execute other terminal-based tasks needed to accomplish user objectives.
...
- Integrates with GGAIIgnore system for file access control
- Handles terminal escape sequences for clean output

## Limitations

- Command access may be restricted by GGAIIgnore rules and security validations
...
1. **Command Validation and Security Checks**:
   ...
   - Checks against GGAIIgnore rules for file access permissions
...
2. **Second Priority: Security Validation**
   ...
   - Commands are checked against GGAIIgnore rules for file access control
...
- When setting up a new project, GG.AI runs initialization commands like `npm init -y` followed by installing dependencies.
- When building a web application, GG.AI executes build commands like `npm run build` to compile assets.
- When deploying code, GG.AI runs git commands to commit and push changes to a repository.
- When troubleshooting, GG.AI executes diagnostic commands to gather system information.
- When starting a development server, GG.AI launches the appropriate server command (e.g., `npm start`).
- When running tests, GG.AI executes the test runner command for the project's testing framework.
