...
The `access_mcp_resource` tool retrieves data from resources exposed by connected Model Context Protocol (MCP) servers. It allows GG.AI to access files, API responses, documentation, or system information that provides additional context for tasks.
...
- When GG.AI needs additional context from external systems
- When GG.AI needs to access domain-specific data from specialized MCP servers
- When GG.AI needs to retrieve reference documentation hosted by MCP servers
- When GG.AI needs to integrate real-time data from external APIs via MCP
...
4. **Response Processing**:
   ...
   - Returns the processed resource data to GG.AI for use in the current task
...
- When helping with API development, GG.AI retrieves endpoint specifications from MCP resources to ensure correct implementation.
- When assisting with data visualization, GG.AI accesses current data samples from connected MCP servers.
- When working in specialized domains, GG.AI retrieves technical documentation to provide accurate guidance.
- When generating industry-specific code, GG.AI references compliance requirements from documentation resources.
