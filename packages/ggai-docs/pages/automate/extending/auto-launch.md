...
Auto-Launch Configuration allows you to automatically start a GG.AI task when opening a workspace, with support for specific profiles and modes. This was originally developed as an internal test feature, but we decided to expose it to users in case anyone finds it useful!
...
When you open a workspace in VS Code, GG.AI automatically checks for a launch configuration JSON file. If found, it:
...
- **`mode`** (string): The GG.AI mode to use for this task. Available modes:
...
- Auto-launch triggers approximately 500ms after GG.AI extension activation
