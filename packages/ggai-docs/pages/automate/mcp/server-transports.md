...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between GG.AI and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (GG.AI) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as GG.AI
...
- Single-client scenarios (one GG.AI instance per server)
...
1. The client (GG.AI) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from GG.AI
...
STDIO servers run locally on the same machine as GG.AI, which has several important implications:
...
- **Execution**: Starts and stops with GG.AI (child process lifecycle)
...
- Start when needed by GG.AI
- Not require network configuration
- Need to be installed alongside GG.AI or via a package manager
...
## Configuring Transports in GG.AI

For detailed information on configuring STDIO and SSE transports in GG.AI, including example configurations, see the [Understanding Transport Types](using-in-kilo-code#understanding-transport-types) section in the Using MCP in GG.AI guide.
