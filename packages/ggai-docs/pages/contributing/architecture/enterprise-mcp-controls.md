...
Enterprise customers need to maintain control over the tools their developers use to ensure security, compliance, and cost management. Developers using GG.AI can configure and use any MCP (Model Context Protocol) server, including public marketplace offerings or arbitrary custom servers. This lack of administrative oversight introduces risk for our enterprise customers, as it allows for the potential use of unvetted, insecure, or costly tool calls.

This document specifies a new feature, **Enterprise MCP Controls**, which allows organization administrators to define an **allowlist** of approved MCP servers. GG.AI (CLI/Extension) can enforce this allowlist, ensuring that developers within the organization can only use sanctioned MCPs.
...
- **Marketplace MCPs:** The dashboard must display a comprehensive list of all MCPs available in the official GG.AI Marketplace.
...
When the Enterprise MCP Controls feature is enabled, extension users can no longer use locally configured MCP definitions. Instead of pulling MCP configurations from the end-user's filesystem, the configuration will be pulled from the GG.AI API, scoped to the organization.
...
For end-users, since the mcp.json payload is no longer configurable locally, they will need to configure it via the GG.AI dashboard. Since these configurations often contain API keys, we will encrypt the entire payload prior to insertion:

```sql
create table if not exists organization_member_mcp_configs (
  id uuid not null default uuid_generate_v4(),
  organization_id uuid not null references organizations(id),
  ggai_user_id text not null references kilocode_users(id),
...
When org users want to configure and use an MCP server and if organizations.settings.mcp_controls_enabled is true, they will be directed to the GG.AI dashboard application `/organizations/:id/mcp-control`. Users will be able to enable, disable, and configure approved MCP servers.
...
When organizations.settings.mcp_controls_enabled is true, the MCP marketplace view should be replaced with a link to configure MCP on the GG.AI dashboard. When it is false-y, the experience is the same as it is today.
...
- Extension
  - When organizations.settings.mcp_controls_enabled is true, the MCP marketplace view should be replaced with a link to configure MCP on the GG.AI dashboard
