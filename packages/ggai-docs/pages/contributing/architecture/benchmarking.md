---
title: "Benchmarking"
description: "Design for benchmarking GG.AI against models and other agents"
---
...
This document proposes a benchmarking system for GG.AI with two primary goals:

1. **Compare models against one another** using the same agent -- measuring task completion, token cost, and total time
2. **Compare agents against one another** using the same model -- e.g., GG.AI vs Claude Code, or GG.AI v1.0 vs v1.1
...
The key engineering deliverable is a **GG.AI Harbor adapter** that runs Kilo CLI autonomously in containerized environments and emits ATIF-compliant trajectories.
...
As GG.AI evolves, we need systematic answers to questions like:
...
- How does GG.AI compare to Claude Code, Codex, or other agents on the same tasks?
...
1. Run GG.AI against standardized benchmark datasets in a reproducible, containerized environment
...
Harbor is the standard evaluation framework used by many frontier labs. Rather than building our own harness, we write a GG.AI adapter and plug into the existing ecosystem.
...
This makes it easy to create task sets that target specific GG.AI capabilities -- for example, a set of refactoring tasks, or a set of multi-file debugging scenarios. Custom sets can be published to the Harbor registry or kept private.

See the [Harbor task tutorial](https://www.tbench.ai/docs/task-tutorial) for a step-by-step guide.

## Deliverables

### 1. GG.AI Harbor Adapter
...
Configure the Opik-Harbor integration for GG.AI benchmark runs:

- Set up `opik harbor run` with the GG.AI adapter
...
Run the same GG.AI agent against Terminal-Bench with different models:
