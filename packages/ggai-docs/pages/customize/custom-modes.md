---
title: "Custom Modes"
description: "Create and configure custom modes in GG.AI"
---

# Custom Modes

GG.AI allows you to create **custom modes** to tailor Kilo's behavior to specific tasks or workflows. Custom modes can be either **global** (available across all projects) or **project-specific** (defined within a single project).

## Sticky Models for Efficient Workflow

Each mode—including custom ones—features **Sticky Models**. This means GG.AI automatically remembers and selects the last model you used with a particular mode. This lets you assign different preferred models to different tasks without constant reconfiguration, as Kilo switches between models when you change modes.
...
{% image src="/docs/img/custom-modes/custom-modes-2.png" alt="Custom mode creation interface in GG.AI" width="600" caption="Custom mode creation interface in GG.AI" /%}

_GG.AI's interface for creating and managing custom modes._
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Slug** (`slug`)                              | A unique internal identifier for the mode. Used by GG.AI to reference the mode, especially for associating mode-specific instruction files.                                      |
| **Name** (`name`)                              | The display name for the mode as it appears in the GG.AI user interface. Should be human-readable and descriptive.                                                               |
...
Modes are managed from the Modes area in GG.AI. Depending on your UI layout, you can open this from the mode selector in the chat panel or from the notebook icon.
...
You can quickly create a basic custom mode by asking GG.AI to do it for you. For example:

```
Create a new mode called "Documentation Writer". It should only be able to read files and write Markdown files.
```

GG.AI will guide you through the process, prompting for necessary information and creating the mode using the preferred YAML format.
...
The interface provides fields for Name, Slug, Description, Save Location, Role Definition, When to Use (optional), Available Tools, and Custom Instructions. After filling these, click the "Create Mode" button. GG.AI will save the new mode in YAML format.

### 3. Manual Configuration (YAML & JSON)

You can directly edit the configuration files to create or modify custom modes. This method offers the most control over all properties. GG.AI now supports both YAML (preferred) and JSON formats.
...
If you see both YAML and JSON mode files, this is usually from legacy configuration. GG.AI reads YAML first and does not keep both files synchronized line-by-line. In practice, edit YAML unless you have a specific reason to stay on JSON.
{% /callout %}
...
- **Purpose:** The display name shown in the GG.AI UI
...
- GG.AI can read `.kilocodemodes` files in either YAML or JSON format
...
You can override GG.AI's built-in modes (like 💻 Code, 🪲 Debug, ❓ Ask, 🏗️ Architect, 🪃 Orchestrator) by creating a custom mode with the same slug.
...
Ready to explore more? Check out the [Show and Tell](https://github.com/genesisgrid/kilocode/discussions/categories/show-and-tell) to discover and share custom modes created by the community!
