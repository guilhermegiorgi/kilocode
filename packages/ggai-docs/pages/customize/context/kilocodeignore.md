...
description: "Control which files GG.AI can access"
---
...
`.kilocodeignore` is a root-level file that tells GG.AI which files and folders it should not access. It uses standard `.gitignore` pattern syntax, but it only affects GG.AI's file access, not Git.

If no `.kilocodeignore` file exists, GG.AI can access all files in the workspace.

## Quick Start

1. Create a `.kilocodeignore` file at the root of your project.
2. Add patterns for files or folders you want GG.AI to avoid.
3. Save the file. GG.AI will pick up the changes automatically.
...
GG.AI checks `.kilocodeignore` before accessing files in tools like:
...
If a file is blocked, GG.AI will return an "access denied" message and suggest updating your `.kilocodeignore` rules.
