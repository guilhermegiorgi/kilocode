---
title: "Skills"
description: "Extend GG.AI capabilities with skills"
---

# Skills

GG.AI implements [Agent Skills](https://agentskills.io/), a lightweight, open format for extending AI agent capabilities with specialized knowledge and workflows.
...
1. **Discovery**: Skills are scanned from designated directories when GG.AI initializes. Only the metadata (name, description, and file path) is read at this stage—not the full instructions.
...
When multiple skills share the same name, GG.AI uses these priority rules:
...
Skills are discovered when GG.AI initializes:

- When VSCode starts
- When you reload the VSCode window (`Cmd+Shift+P` → "Developer: Reload Window")

Skills directories are monitored for changes to `SKILL.md` files. However, the most reliable way to pick up new skills is to reload VS or the GG.AI extension.
...
In GG.AI, the `name` field **must match** the parent directory name:
...
- **Kilo Marketplace** - Browse skills directly in the GG.AI extension via the Marketplace tab, or explore the [Kilo Marketplace repository](https://github.com/genesisgrid/kilo-marketplace) on GitHub
...
1. **Check the Output panel**: Open `View` → `Output` → Select "GG.AI" from dropdown. Look for skill-related errors.
...
Have you created a skill that others might find useful? Share it with the community by contributing to the [Kilo Marketplace](https://github.com/genesisgrid/kilo-marketplace)!
