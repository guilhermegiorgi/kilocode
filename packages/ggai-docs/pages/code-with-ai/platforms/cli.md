---
title: "Kilo CLI"
description: "Using GG.AI from the command line"
---
...
The GG.AI CLI uses the same underlying technology that powers the IDE extensions, so you can expect the same workflow to handle agentic coding tasks from start to finish.

**Source code & issues (Kilo CLI 1.0):** [genesisgrid/kilocode](https://github.com/genesisgrid/kilocode) · [Report an issue](https://github.com/genesisgrid/kilocode/issues)
...
## What you can do with GG.AI CLI
...
GG.AI uses the permission config to decide whether a given action should run automatically, prompt you, or be blocked.
...
```json
{
  "$schema": "https://gg.ai/config.json",
  "permission": {
    "*": "ask",
    "bash": "allow",
    "edit": "deny"
...
The Kilo CLI is a fork of [OpenCode](https://opencode.ai) and supports the same configuration options. The CLI you install with `npm install -g @kilocode/cli` (Kilo CLI 1.0) is built from [genesisgrid/kilocode](https://github.com/genesisgrid/kilocode). For comprehensive configuration documentation, see the [OpenCode Config documentation](https://opencode.ai/docs/config).
...
Interactive mode is the default mode when running GG.AI without the `--auto` flag, designed to work interactively with a user through the console.

In interactive mode GG.AI will request approval for operations which have not been auto-approved, allowing the user to review and approve operations before they are executed, and optionally add them to the auto-approval list.
...
Autonomous mode allows GG.AI to run in automated environments like CI/CD pipelines without requiring user interaction.
...
- `GGAI_PROVIDER`: Override the active provider ID
- For `kilocode` provider: `KILOCODE_<FIELD_NAME>` (e.g., `KILOCODE_MODEL` → `kilocodeModel`)
- For other providers: `GGAI_<FIELD_NAME>` (e.g., `GGAI_API_KEY` → `apiKey`)
