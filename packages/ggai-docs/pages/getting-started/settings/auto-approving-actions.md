...
description: "Configure automatic approval settings for GG.AI operations"
---

# Auto-Approving Actions

{% callout type="danger" %}
**Security Warning:** Auto-approve settings bypass confirmation prompts, giving GG.AI direct access to your system. This can result in data loss, file corruption, or worse. Command line access is particularly dangerous, as it can potentially execute harmful operations that could damage your system or compromise security. Only enable auto-approval for actions you fully trust.
{% /callout %}
...
2. Select which actions GG.AI can perform without asking permission
3. Use the master toggle (leftmost checkbox) to quickly enable/disable all permissions

[![GGAI Task Timeline](https://img.youtube.com/vi/NBccFnYDQ-k/maxresdefault.jpg)](https://youtube.com/shorts/NBccFnYDQ-k?feature=shared)
...
| ------------------------------ | ------------------------------------------------ | ----------- |
| **Read files and directories** | Lets GG.AI access files without asking       | Medium      |
| **Edit files**                 | Lets GG.AI modify files without asking       | **High**    |
...
| **Use MCP servers**            | Lets GG.AI use configured MCP services       | Medium-High |
| **Switch modes**               | Changes between GG.AI modes automatically    | Low         |
...
{% callout type="info" %}
Allow GG.AI to automatically perform operations without requiring approval. Enable these settings only if you fully trust the AI and understand the associated security risks.
{% /callout %}
...
**Description:** When enabled, GG.AI will automatically view directory contents and read files without requiring you to click the Approve button.

**Risk level:** Medium

While this setting only allows reading files (not modifying them), it could potentially expose sensitive data. Still recommended as a starting point for most users, but be mindful of what files GG.AI can access.

#### Read Outside Workspace

**Setting:** "Allow reading files outside the workspace"

**Description:** When enabled, GG.AI can read files outside the current workspace directory without asking for approval.

**Risk level:** Medium-High

This setting extends read permissions beyond your project folder. Consider the security implications:

- GG.AI could access sensitive files in your home directory
...
**Recommendation:** Keep disabled unless you specifically need GG.AI to read files outside your project.
...
This setting allows GG.AI to modify your files without confirmation. The delay timer is crucial:
...
**Description:** When enabled, GG.AI can create or modify files outside the current workspace directory without asking for approval.

**Risk level:** Very High

Use with caution and in controlled environments. It allows GG.AI to:
...
**Description:** When enabled, GG.AI can overwrite or modify files that are normally protected by the [`.kilocodeignore`](/docs/customize/custom-rules) file.
...
This setting allows GG.AI to permanently remove files without confirmation.

**Safeguards:**

- GG.AI still respects `.kilocodeignore` rules
...
Allows GG.AI to control a headless browser without confirmation. This can include:
...
The delay controls how long GG.AI waits before trying again:
...
Allows GG.AI to change between different modes (Code, Architect, etc.) without asking for permission. This primarily affects the AI's behavior rather than system access.
...
Enables GG.AI to create and complete subtasks automatically. This relates to workflow organization rather than system access.
...
Add command prefixes (e.g., `git`, `npm`, `ls`) that GG.AI can run without asking. Use `*` to allow all commands (use with caution).
...
**Description:** Automatically selects the first AI-suggested answer for a follow-up question after a configurable timeout. This speeds up your workflow by letting GG.AI proceed without manual intervention.
...
- Overnight runs where you want GG.AI to continue working
...
This setting allows GG.AI to automatically update task progress and todo lists during work sessions. This includes:
...
This is particularly useful when combined with the Subtasks permission, as it allows GG.AI to maintain a complete picture of project progress without constant approval requests.
...
**"You Only Live Once"** mode enables _all_ auto-approve permissions at once using the master toggle. This gives GG.AI complete autonomy to read files, write code, execute commands, and perform any operation without asking for permission.
...
This is the fastest way to work with GG.AI, but also the riskiest. Use it only when you fully trust the AI and are prepared for the consequences.
{% /callout %}
