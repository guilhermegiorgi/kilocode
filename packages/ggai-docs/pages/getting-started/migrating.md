...
description: "Guide for migrating to GG.AI from other AI coding tools"
---

# Migrating from Cursor or Windsurf

Quickly migrate your custom rules from Cursor or Windsurf to GG.AI. The process typically takes just a few minutes per project.

{% callout type="info" title="Two Workflow Approaches"%}

GG.AI supports **two complementary workflows**—choose the one that fits your style, or use both:
...
## Why GG.AI's Rules System?

GG.AI simplifies AI configuration while adding powerful new capabilities:
...
### What's Different in GG.AI

| Cursor                                      | GG.AI                                 | Key Difference                              |
| ------------------------------------------- | ----------------------------------------- | ------------------------------------------- |
...
Cursor's `globs` field specifies which files a rule applies to. GG.AI handles this through **mode-specific directories** instead.
...
**GG.AI approach (Option 1 - Mode-specific directory):**
...
**GG.AI approach (Option 2 - Custom mode):**
...
Cursor supports nested `.cursor/rules/` directories. GG.AI uses flat structure with descriptive names:

```bash
# Cursor: .cursor/rules/backend/server/api-rules.mdc
# GG.AI: .kilocode/rules/backend-server-api-rules.md
```
...
Windsurf configures activation through the GUI. In GG.AI, file organization replaces GUI configuration:

| Windsurf GUI Mode        | GG.AI Equivalent                                        |
| ------------------------ | ----------------------------------------------------------- |
...
All three tools support the `AGENTS.md` standard. If you have one, it works in GG.AI automatically:

```bash
# Verify it exists
ls -la AGENTS.md

# That's it - GG.AI loads it automatically (enabled by default)
```

**Important:** Use uppercase `AGENTS.md` (not `agents.md`). GG.AI also accepts `AGENT.md` (singular) as a fallback.

**Note:** Both `AGENTS.md` and `AGENT.md` are write-protected files in GG.AI and require user approval to modify.

## Understanding Mode-Specific Rules

This is GG.AI's unique feature that replaces both Cursor's `globs` and Windsurf's activation modes.
...
- [ ] **Verify rules loaded:** Click law icon (⚖️) in GG.AI panel
- [ ] **Test rule application:** Ask GG.AI to perform tasks following your rules
...
- **After migrating:** Organize files accordingly in GG.AI

### Nested Rules Flattened

Cursor's nested directories don't map to GG.AI. Flatten with descriptive names:
...
- **Check setting:** Verify "Use Agent Rules" is enabled in GG.AI settings (enabled by default)
- **Reload:** Restart VS Code if needed

### Choosing Your Workflow

GG.AI supports **both autocomplete and chat-driven workflows**. Choose the approach that fits your coding style, or combine them:
...
- [Join our Discord](https://gg.ai/discord) for migration support
