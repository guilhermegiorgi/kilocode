...
In January 2026, Anthropic decided to restrict Claude Code CLI to official Claude Code clients. Claude Code credentials cannot be used in GG.AI or other third-party harnesses.

For continued use of Anthropic models in GG.AI, please use the [Anthropic API provider](/docs/ai-providers/anthropic) with an API key instead.
{% /callout %}

# Using Claude Code With GG.AI

Claude Code is Anthropic's official CLI that provides direct access to Claude models from your terminal. Using Claude Code with GG.AI lets you leverage your existing CLI setup without needing separate API keys.
...
If this environment variable is set on your system, the `claude` tool may use it for authentication instead of the interactive `/login` method. When GG.AI executes the tool, it will accurately reflect that an API key is being used, as this is the underlying behavior of the `claude` CLI itself.
{% /callout %}
...
1. **Open GG.AI Settings:** Click the gear icon ({% codicon name="gear" /%}) in the GG.AI panel.
...
- **Enhance Prompt Feature:** Full compatibility with GG.AI's Enhance Prompt feature, allowing you to automatically improve and refine your prompts before sending them to Claude.
