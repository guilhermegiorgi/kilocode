...
# Using OpenAI Compatible Providers With GG.AI

GG.AI supports a wide range of AI model providers that offer APIs compatible with the OpenAI API standard. This means you can use models from providers _other than_ OpenAI, while still using a familiar API interface. This includes providers like:
...
You'll find these settings in the GG.AI settings panel (click the {% codicon name="gear" /%} icon):
...
GG.AI supports full endpoint URLs in the Base URL field, providing greater flexibility for provider configuration:
...
By using an OpenAI-compatible provider, you can leverage the flexibility of GG.AI with a wider range of AI models. Remember to always consult your provider's documentation for the most accurate and up-to-date information.
