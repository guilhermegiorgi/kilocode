...
The Human Relay provider allows you to use GG.AI with web-based AI models like ChatGPT or Claude without needing an API key. Instead, it relies on you to manually relay messages between GG.AI and the AI's web interface.

## How it Works

1.  **Select Human Relay**: Choose "Human Relay" as your API provider in GG.AI's settings. No API key is required.
2.  **Initiate a Request**: Start a chat or task with GG.AI as usual.
...
6.  **Paste Back to GG.AI**: Return to the dialog box in VS Code, paste the AI's response into the designated field, and click "Confirm".
7.  **Continue**: GG.AI will process the response as if it came directly from an API.
