...
const kilo = createOpenAI({
  baseURL: "https://api.gg.ai/api/gateway",
  apiKey: process.env.GGAI_API_KEY,
})
...
client = OpenAI(
    ...
    base_url="https://api.gg.ai/api/gateway",
)
...
```bash
curl -X POST "https://api.gg.ai/api/gateway/chat/completions" \
...
```bash
curl -N -X POST "https://api.gg.ai/api/gateway/chat/completions" \
  -H "Authorization: Bearer $GGAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "anthropic/claude-sonnet-4.5",
...
func main() {
    ...
    req, _ := http.NewRequest("POST",
        "https://api.gg.ai/api/gateway/chat/completions",
        bytes.NewBuffer(jsonBody))

    req.Header.Set("Authorization", "Bearer "+os.Getenv("GGAI_API_KEY"))
...
uri = URI('https://api.gg.ai/api/gateway/chat/completions')
http = Net::HTTP.new(uri.host, uri.port)
http.use_ssl = true

request = Net::HTTP::Post.new(uri)
request['Authorization'] = "Bearer #{ENV['GGAI_API_KEY']}"
...
const model = new ChatOpenAI({
  ...
  openAIApiKey: process.env.GGAI_API_KEY,
  configuration: {
    baseURL: "https://api.gg.ai/api/gateway",
  },
})
