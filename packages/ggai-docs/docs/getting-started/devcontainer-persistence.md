...
description: How to preserve GG.AI threads and settings in dev containers
---

# Dev Container Persistence

When using GG.AI in development containers (VS Code Dev Containers, GitHub Codespaces, etc.), your threads and settings can persist across container rebuilds by properly configuring volume mounts.

## Why Persistence Matters

Dev containers are ephemeral by default - when you rebuild the container, all data is lost unless explicitly persisted. GG.AI stores important data including:

- **Conversation threads**: Your ongoing discussions with GG.AI
- **Settings**: API configurations, custom modes, and preferences
- **Cache**: Vector store for code indexing and browser tool data

## Required Configuration

The GG.AI dev container is pre-configured with named volumes to preserve your data. If you're setting up your own dev container, add these mounts to your `devcontainer.json`:
...
2. Start new conversations with GG.AI
...
2. Set the custom storage path in VS Code settings:
   ...
   - Search for "GG.AI: Custom Storage Path"
...
GitHub Codespaces automatically persists your VS Code settings and extensions. For GG.AI threads, the pre-configured dev container includes the necessary volume mounts.
