...
import { GGAI_API_BASE, POLL_INTERVAL_MS } from "../api/constants.js"
import type { AuthOuathResult } from "@ggai/plugin"
...
async function initiateDeviceAuth(): Promise<DeviceAuthInitiateResponse> {
  const response = await fetch(`${GGAI_API_BASE}/api/device-auth/codes`, {
    method: "POST",
    headers: {
...
async function pollDeviceAuth(code: string): Promise<DeviceAuthPollResponse> {
  const response = await fetch(`${GGAI_API_BASE}/api/device-auth/codes/${code}`)
