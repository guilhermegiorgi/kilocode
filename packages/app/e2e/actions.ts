...
export async function openSessionMoreMenu(page: Page, sessionID: string) {
  ...
  await page.waitForTimeout(100) // ggai_change - wait before menu trigger click
...
export async function clickMenuItem(menu: Locator, itemName: string | RegExp, options?: { force?: boolean }) {
  ...
  await item.waitFor({ state: "visible", timeout: 5000 }) // ggai_change - wait for menu item to be visible in case of animation
  await menu.page().waitForTimeout(100) // ggai_change - wait for menu animation
...
export async function setWorkspacesEnabled(page: Page, projectSlug: string, enabled: boolean) {
  ...
  await page.waitForTimeout(100) // ggai_change - wait before toggle click
...
export async function openWorkspaceMenu(page: Page, workspaceSlug: string) {
  ...
  await expect(item).toBeVisible()
  await item.hover()
  await page.waitForTimeout(200) // ggai_change - wait for hover state

  const trigger = page.locator(workspaceMenuTriggerSelector(workspaceSlug)).first()
  await expect(trigger).toBeVisible()
  await page.waitForTimeout(100) // ggai_change - wait before trigger click
  await trigger.click({ force: true })
