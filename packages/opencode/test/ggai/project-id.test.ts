...
import { getKiloProjectId } from "../../src/ggai/project-id"
  ...
  describe("normalization", () => {
    test("extracts repo name from HTTPS git URL", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.$`git remote add origin https://github.com/genesisgrid/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from SSH git URL", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.$`git remote add origin git@github.com:genesisgrid/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from HTTPS URL without .git extension", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com/genesisgrid/handbook`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from ssh:// URL", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin ssh://git@github.com/genesisgrid/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("truncates long repo names to 100 characters", async () => {
      ...
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com/genesisgrid/${longName}.git`.cwd(dir).quiet()
        },
      })
      ...
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.write(
            ...
            JSON.stringify({
              project: {
                id: "https://github.com/genesisgrid/another-repo.git",
              },
            }),
          )
        },
  ...
  describe("edge cases", () => {
    test("handles git URLs with port numbers", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com:443/genesisgrid/handbook.git`.cwd(dir).quiet()
        },
      })
