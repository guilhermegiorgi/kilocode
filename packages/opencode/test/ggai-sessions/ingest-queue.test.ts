...
import { IngestQueue } from "../../src/ggai-sessions/ingest-queue"
...
describe("share ingest queue", () => {
  ...
  test("ggai_meta uses stable key and coalesces", async () => {
    const sent: unknown[] = []
    const sched = scheduler(() => clock.now)
    ...
    await q.sync("s7", [
      { type: "ggai_meta", data: { platform: "cli", gitUrl: "https://github.com/old/repo.git", gitBranch: "main" } },
    ])
    clock.now = 100
    await q.sync("s7", [
      {
    ...
    expect(sent.length).toBe(1)
    expect((sent[0] as any).data.length).toBe(1)
    expect((sent[0] as any).data[0].type).toBe("ggai_meta")
  ...
  test("flush sends request with ?v=1 query parameter", async () => {
    ...
    const q = IngestQueue.create({
      ...
      getClient: async () => ({
        url: "https://ingest.test",
        fetch: async (input: any, _init: any) => {
