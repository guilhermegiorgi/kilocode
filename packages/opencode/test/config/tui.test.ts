...
afterEach(async () => {
  delete process.env.GGAI_CONFIG
  delete process.env.GGAI_TUI_CONFIG
...
test("project config takes precedence over GGAI_TUI_CONFIG (matches GGAI_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.GGAI_TUI_CONFIG = custom
    },
  })
...
test("GGAI_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from GGAI_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.GGAI_CONFIG = path.join(customDir, "opencode.json")
    },
  })
