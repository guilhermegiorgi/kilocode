...
import { RulesMigrator } from "./rules-migrator" // ggai_change
import { WorkflowsMigrator } from "./workflows-migrator"
import { IgnoreMigrator } from "./ignore-migrator" // ggai_change
...
export namespace KilocodeConfigInjector {
  ...
  export async function buildConfig(options: {
    ...
    // ggai_change start - Rules migration
    ...
    // ggai_change start - Ignore migration
  ...
  export function getEnvVars(configJson: string): Record<string, string> {
    ...
    return {
      GGAI_CONFIG_CONTENT: configJson,
    }
  }
}
