...
import { IngestQueue } from "@/ggai-sessions/ingest-queue"
import { clearInFlightCache, withInFlightCache } from "@/ggai-sessions/inflight-cache"
import type * as SDK from "@ggai/sdk/v2"
import z from "zod"
import { GGAI_API_BASE } from "@ggai/gateway"
import { Instance } from "@/project/instance"
import { Vcs } from "@/project/vcs"
import simpleGit from "simple-git"


export namespace GGAISessions {
  const log = Log.create({ service: "ggai-sessions" })

  const Uuid = z.uuid()
  type Uuid = z.infer<typeof Uuid>

  const tokenValidKeyTemplate = "ggai-sessions:token-valid:"
  let tokenValidKey = tokenValidKeyTemplate + "unknown"


  const tokenKey = "ggai-sessions:token"
  const orgKey = "ggai-sessions:org"
  const clientKey = "ggai-sessions:client"
  const gitUrlKeyPrefix = "ggai-sessions:git-url:"
  ...
  async function authValid(token: string) {
    ...
    return withInFlightCache(tokenValidKey, 15 * 60_000, async () => {
      const response = await fetch(`${GGAI_API_BASE}/api/user`, {
        headers: {
  ...
  async function getClient(): Promise<Client | undefined> {
    return withInFlightCache(clientKey, ttlMs, async () => {
      ...
      const base = process.env["GGAI_SESSION_INGEST_URL"] ?? "https://ingest.kilosessions.ai"
  ...
  const shareDisabled = process.env["GGAI_DISABLE_SHARE"] === "true" || process.env["GGAI_DISABLE_SHARE"] === "1"
  const ingestDisabled =
    process.env["GGAI_DISABLE_SESSION_INGEST"] === "true" || process.env["GGAI_DISABLE_SESSION_INGEST"] === "1"
  const debugIngest =
    process.env["GGAI_DEBUG_SESSION_INGEST"] === "true" || process.env["GGAI_DEBUG_SESSION_INGEST"] === "1"
  ...
  export async function share(sessionId: string) {
    if (ingestDisabled) {
      throw new Error("Session ingest is disabled (GGAI_DISABLE_SESSION_INGEST=1)")
    }

    if (shareDisabled) {
      throw new Error("Sharing is disabled (GGAI_DISABLE_SHARE=1)")
    }
    ...
    const url = `https://app.gg.ai/s/${result.public_id}`
  ...
  export async function unshare(sessionId: string) {
    ...
    if (shareDisabled) {
      throw new Error("Unshare is disabled (GGAI_DISABLE_SHARE=1)")
    }
  ...
  async function meta(sessionId?: string) {
    ...
    const platform = override || process.env["GGAI_PLATFORM"] || "cli"
  ...
  async function getOrgId(): Promise<Uuid | undefined> {
    const env = process.env["GGAI_ORG_ID"]
