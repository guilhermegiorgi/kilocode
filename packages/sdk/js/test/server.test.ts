// ggai_change start - Tests for GGAI_CONFIG_CONTENT merging
import { describe, test, expect, beforeEach, afterEach } from "bun:test"
import { buildConfigEnv } from "../src/server"

describe("buildConfigEnv", () => {
  const originalEnv = process.env.GGAI_CONFIG_CONTENT

  beforeEach(() => {
    delete process.env.GGAI_CONFIG_CONTENT
  })
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.GGAI_CONFIG_CONTENT = originalEnv
    }
  })
  ...
  test("preserves existing GGAI_CONFIG_CONTENT when spawning with new config", () => {
    // Simulate Kilocode having injected modes via GGAI_CONFIG_CONTENT
    process.env.GGAI_CONFIG_CONTENT = JSON.stringify({
      agent: {
  ...
  test("handles invalid JSON in existing GGAI_CONFIG_CONTENT gracefully", () => {
    process.env.GGAI_CONFIG_CONTENT = "invalid json {"
