...
**Issue:** [#6089](https://github.com/genesisgrid/kilocode/issues/6089)

## Problem

Users upgrading from the old GG.AI extension have their settings stored in VS Code's `globalState` under the old extension's keys. The new extension uses the CLI's `opencode.json` for configuration. Without migration, users lose all their settings (API keys, model preferences, auto-approve rules, etc.) on upgrade.
