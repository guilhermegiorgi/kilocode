# Feature Parity Plan — GG.AI VS Code Extension (Rebuild)

## Overview

This extension is a **ground-up rebuild** of the [old GG.AI extension](https://github.com/genesisgrid/kilocode-legacy) using Kilo CLI as the backend. Rather than migrating the old extension's codebase, we started fresh with a Solid.js webview, a CLI server manager, and a message-based protocol between extension host and webview. This new extension lives in the [kilocode monorepo](https://github.com/genesisgrid/kilocode/tree/main/packages/kilo-vscode).

This document tracks remaining work needed for feature parity with the old extension. Each feature links to its detailed parity requirement doc. Features sourced from the [GitHub project board](https://github.com/orgs/genesisgrid/projects/25/views/1) include issue links.
...
Open issues from the [GitHub project board](https://github.com/orgs/genesisgrid/projects/25/views/1) not covered by the feature docs above. Each item has its own detailed doc.
...
| [Dedicated Output Channel](infrastructure/dedicated-output-channel.md)     | 🔨 Partial     | General "GG.AI" output channel (Agent Manager has its own already). Centralized logging utility.                                           | Extension (new logger utility)            | P2       |
...
- [ ] **Dedicated output channel** — All logging currently goes to `console.log` mixed with other extensions ([details](infrastructure/dedicated-output-channel.md)). Create a dedicated "GG.AI" output channel before production
