...
export class ServerManager {
  ...
  private async startServer(): Promise<ServerInstance> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const serverProcess = spawn(cliPath, ["serve", "--port", "0"], {
        env: {
          ...process.env,
          GGAI_SERVER_PASSWORD: password,
          GGAI_CLIENT: "vscode",
          GGAI_ENABLE_QUESTION_TOOL: "true",
          GGAICODE_FEATURE: "vscode-extension", // ggai_change - feature tracking
          GGAI_TELEMETRY_LEVEL: vscode.env.isTelemetryEnabled ? "all" : "off",
          GGAI_APP_NAME: "kilo-code",
          GGAI_EDITOR_NAME: vscode.env.appName,
          GGAI_PLATFORM: "vscode",
          GGAI_MACHINE_ID: vscode.env.machineId,
          GGAI_APP_VERSION: this.context.extension.packageJSON.version,
          GGAI_VSCODE_VERSION: vscode.version,
        },
  ...
  private getCliPath(): string {
    ...
    const binName = process.platform === "win32" ? "ggai.exe" : "ggai" // ggai_change
