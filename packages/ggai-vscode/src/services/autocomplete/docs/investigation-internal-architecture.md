...
**LLM abstraction layer** — bridges autocomplete requests to the GG.AI API system.
...
8. **LLM providers** — `ILLM` interface with implementations for OpenAI, Mistral, OpenRouter, GGAI, Mock.
...
Provides **autocomplete for the chat input text area** in the GG.AI webview. When the user is typing a message in the chat panel, this module suggests completions for natural language text.
