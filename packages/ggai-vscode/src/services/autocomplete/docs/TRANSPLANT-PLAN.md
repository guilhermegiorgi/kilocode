...
This plan defines those interfaces and the minimum VS Code extension scaffolding required, without coupling the new extension to GG.AI’s current provider implementations.
...
Current GG.AI activates broadly:
...
Commands declared in GG.AI’s `package.json` (some are placeholders):
...
- Register a `CodeActionsProvider` programmatically (as GG.AI does)
...
GG.AI does **not** contribute `configuration` settings; it stores everything in global state.
...
Most modifications are to replace GG.AI specific imports with the new interfaces.
...
- Uses internal GG.AI wrapper and webview provider.
...
- If you do, implement a dedicated transport layer; keep the bridge logic but replace:
  - `ClineProvider`
  - `getGGAIWrapperProperties`
  - any GG.AI-specific types
...
GG.AI’s `package.json` command titles reference NLS keys like:
...
Runtime strings are accessed via `t()` from `src/i18n` (GG.AI).
...
3. Refactor `index.ts` entry point to accept dependency injection rather than GG.AI’s provider.
