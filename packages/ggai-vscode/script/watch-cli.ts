 ...
 * then copies it into packages/kilo-vscode/bin/ggai.
 *
...
const targetBinPath = join(targetBinDir, "ggai")
...
function sourceBinaryPath(): string {
  return join(opencodeDir, "dist", `@kilocode/cli-${process.platform}-${process.arch}`, "bin", "ggai")
}
...
async function rebuild() {
  ...
  try {
    ...
    log(`Binary updated (${elapsed}s): ${relative(packagesDir, source)} -> bin/ggai`)
  } catch (err) {
