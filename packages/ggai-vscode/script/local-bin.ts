...
/**
 * Ensures the VS Code extension has a CLI binary at `packages/kilo-vscode/bin/ggai`.
 *
 * Strategy:
 * 1) If `bin/ggai` already exists -> ok.
 ...
 * 4) Copy the resulting binary into `packages/kilo-vscode/bin/ggai` and chmod +x.
 *
...
const binName = process.platform === "win32" ? "ggai.exe" : "ggai"
...
async function findKiloBinaryInOpencodeDist(): Promise<string | null> {
  ...
  // Fallback: find any dist/**/bin/ggai or ggai.exe
  const queue = [distDir]
  while (queue.length) {
    const dir = queue.pop()
    if (!dir) continue
    ...
    for (const e of entries) {
      ...
      if (e.isFile() && (e.name === "ggai" || e.name === "ggai.exe") && basename(dirname(p)) === "bin") {
        return p
      }
    }
  }
...
async function ensureBuiltBinary(): Promise<string> {
  ...
  if (!built) {
    throw new Error(
      `CLI build completed but no binary was found in ${join(opencodeDir, "dist")} (expected dist/**/bin/ggai).`,
    )
  }
  return built
}
